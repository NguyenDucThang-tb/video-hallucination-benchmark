...
def parse_args():
    ...
    parser.add_argument("--video_path", help="Path to the video files.", default=None)
    parser.add_argument("--output_dir", help="Directory to save the model results JSON.", default='./')
    parser.add_argument("--output_name", help="Name of the file for storing results JSON.", default=None)
    parser.add_argument("--model-path", type=str, default="lmms-lab/LLaVA-NeXT-Video-34B")
...
def load_video(video_path, args):
    ...
    while success:
        ...
        frame_count += 1
    print(frame_count, len(vr))
    if len(vr) > frame_count * 1.8:
...
def run_inference(args, cfg_pretrained, tokenizer, model, image_processor, context_len):
    """
    ...
    for video_path in all_video_pathes:
        ...
        if "gpt4v" != args.model_path:
            ...
            with torch.inference_mode():
                ...
                # import pdb;pdb.set_trace()
                # output_ids = model.generate(inputs=input_ids, images=video, attention_mask=attention_masks, modalities="video", do_sample=True, temperature=0.0, max_new_tokens=1024, use_cache=True, stopping_criteria=[stopping_criteria])
...
if __name__ == "__main__":
    ...
    results = {}
    for index, video_dict in videos_dict.items():
        if index in results:
            continue
        temp = {}
        video_id = video_dict['video'].rsplit('_', 3)[0]

        # Find the video path by traversing the directory
        video_path = None
        for root, dirs, files in os.walk(video_folder):
            for file in files:
                if file == f"{video_dict['video']}.mp4":
        ...
        if not video_path:
            print(f"Video file {clip_name}.mp4 not found.")
            continue
        ...
        # Call main()
        inp = question
        inp += "Sort these two actions in the order they occur in the video, and return which action happen before which one. If you only detect one action, return that action."
        ...
        model_answer = run_inference(args, cfg_pretrained, tokenizer, model, image_processor, context_len)
        ...
        print(correct_answer, model_answer)
  
        with open('LLaVA-Video_Hallu_TSH.json', 'w') as f:
